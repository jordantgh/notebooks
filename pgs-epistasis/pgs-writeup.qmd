...
To model this effectively we can use log odds and log odds ratios. We shall assume a baseline disease prevalence of 1% and convert the relevant quantities to log space.
...
data = {
    'Parameter': ['ln(C)', 'ln(OR_alpha)', 'ln(OR_beta)', 'ln(OR_alpha_beta)'],
    'Value': [ln_C, ln_OR_a, ln_OR_b, ln_OR_ab]
}

df = pd.DataFrame(data)
df['Value'] = df['Value'].round(4)

HTML(df.round(3).to_html(index=False))
```
...
def log_odds(a, b, epistasis_dummy=1):
    return ln_C + ln_OR_a * a + ln_OR_b * b + ln_OR_ab * a * b * epistasis_dummy
...
for genotype, (a, b, freq, n_pop, epistasis_dummy) in genotypes.items():
    odds = np.exp(log_odds(a, b, epistasis_dummy))
    prevalence = 100 * odds / (1 + odds)
    n_cases = round(prevalence * n_pop)
    results.append([genotype, prevalence, freq, n_pop, n_cases])
...
prevalence = 0.01
C = prevalence / (1 - prevalence)
ln_C = np.full(n_individuals, np.log(C))

def array_log_odds(genotypes, snp_df, ln_C):
    ln_odds = ln_C.copy()
    ln_odds_no_epi = ln_C.copy()
...
genotypes['prob_with_epistasis'] = (
    genotypes['odds_with_epi'] / (1 + genotypes['odds_with_epi'])
)

genotypes['prob_no_epistasis'] = (
    genotypes['odds_no_epistasis'] / (1 + genotypes['odds_no_epistasis'])
)
...
with ThreadPoolExecutor() as executor:
    future_to_params = {
        executor.submit(
            ...
            maf=0.5,
            n_reps=n_reps
        ): i for i, row in variances.iterrows()
    }
...
r = mean_results[
    mean_results['additive_variance'] == 0.45
]['mean_adj_r_squared'].mean()
