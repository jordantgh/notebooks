...
On its own, this is not very informative. The `preds` object contains the predictions for each row of the data, but they are not on a scale that is immediately interpretable. We need to add back the intercept and the effect of $Z$ to get the "normalised" $Y$.
